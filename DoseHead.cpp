...
DoseSchedule* schedules[] = {
  &Alk_Schedule, &Ca_Schedule};  
...
void doDoseStateUI(){
  for(int i=0; i<NUMBER_OF_PUMPS; i++){
    if(getSchedule(i)->isPumpRunning()){
      ...
      unsigned long timeRemaining = (getSchedule(i)->getPumpRunTime() - (millis() - getSchedule(i)->getPumpStartTime())) / 1000;
      ...
      if(currentState == ALERT_STATE){  // if the state hasn't changed. 
        if(numberOfAlerts > 0){  // if there are any exceptions at all left.
          if(peekRotaryEncoder()){
            useRotaryEncoder(currentAlertIndex, 0, numberOfAlerts -1, 1);
